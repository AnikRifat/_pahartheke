                    ...
                    <div class="row aiz-steps arrow-divider">
                        <div class="col done">
                            <div class="text-center text-success">
                                ...
                                <h3 class="fs-14 fw-600 d-none d-lg-block text-capitalize">{{ translate('1. Shipping info')}}</h3>
                            </div>
                        </div>
                        <div class="col done">
                            <div class="text-center text-success">
                                <i class="la-3x mb-2 las la-credit-card"></i>
                                <h3 class="fs-14 fw-600 d-none d-lg-block text-capitalize">{{ translate('2. Payment')}}</h3>
                            </div>
                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ single_price(( $order->orderDetails->sum('price') - $order->total_discount ) + $order->orderDetails->sum('tax')) }}</td>
                                            </tr>
                                            <tr>
                                        ...
                                        <table class="table ">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <th>Discount</th>
                                                    <td class="text-right">
                                                        <span class="fw-600">{{ single_price($order->total_discount) }}</span>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th>{{ translate('Subtotal')}}</th>
                                                    <td class="text-right">
                                                        <span class="fw-600">{{ single_price($order->orderDetails->sum('price')  - $order->total_discount) }}</span>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        ...
                                                        @php
                                                            $get_shipping_cost = \DB::table('order_details')->where('order_id', $order->id)->first('shipping_cost');
                                                        @endphp

                                                        <span class="font-italic">{{ single_price($get_shipping_cost->shipping_cost) }}</span>
                                                       
                                                        {{-- <span class="font-italic">{{ single_price(round()) }}</span> --}}
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th>{{ translate('Tax')}}</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <strong><span>{{ single_price($order->grand_total - $order->total_discount) }}</span></strong>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
...
@section('fbq')
<script>
         fbq('track', 'Purchase', {value: <?php echo floor(($order->orderDetails->sum('price') + $order->orderDetails->sum('shipping_cost')) / 95)?>, currency: 'USD'});
    </script>
@endsection